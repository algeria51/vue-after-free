...
;(function () {
  ...
  const RW = SW - PAD * 2; const SY = HDR + 10
  const VOFF = Math.floor(RW * 0.60); const HOFF = Math.floor(RW * 0.78)
  ...
  jsmaf.onKeyDown = function (kc: number) {
    if (kc === 6 || kc === 5) { cur = (cur + 1) % TOTAL; sfxCur(); clamp(); renderRows() } else if (kc === 4 || kc === 7) { cur = (cur - 1 + TOTAL) % TOTAL; sfxCur(); clamp(); renderRows() } else if (kc === confirmKey) { sfxOk(); onPress() } else if (kc === backKey) { sfxBack(); saveConfig(function () { try { include('themes/' + (typeof CONFIG !== 'undefined' && CONFIG.theme ? CONFIG.theme : 'default') + '/main.js') } catch (e) { log('Back: ' + (e as Error).message) } }) }
  }
