...
;(function () {
  ...
  function sfx (url: string) {
    ...
    try { const cl = new jsmaf.AudioClip(); cl.volume = 1.0; cl.open(url) } catch (_e) {}
  }
  ...
  new Style({ name: 'secbadge', color: 'rgba(80,210,255,0.70)',    size: 11 })
  new Style({ name: 'val',       color: 'rgb(120,210,255)',        size: 20 })
  new Style({ name: 'selval',    color: 'rgb(60,230,255)',         size: 20 })
  new Style({ name: 'toggle_on', color: 'rgb(80,230,150)',         size: 20 })
  new Style({ name: 'toggle_off',color: 'rgba(255,110,110,0.80)', size: 20 })
  new Style({ name: 'arrow',     color: 'rgba(255,255,255,0.28)',  size: 20 })
  new Style({ name: 'arrsel',    color: 'rgb(60,230,255)',         size: 20 })
  new Style({ name: 'hint',      color: 'rgba(255,255,255,0.24)',  size: 14 })
  ...
  ttl.x = PAD_X; ttl.y = 34; ttl.style = 'title'
  jsmaf.root.children.push(ttl)

  const subTxt = new jsmaf.Text()
  subTxt.text = TOTAL + ' settings'
  subTxt.x = PAD_X; subTxt.y = 86; subTxt.style = 'count'
  jsmaf.root.children.push(subTxt)

  // Column headers
  const hdrOpt = new jsmaf.Text()
  hdrOpt.text = 'OPTION'; hdrOpt.x = PAD_X + 18; hdrOpt.y = HEADER_H + 2; hdrOpt.style = 'colhdr'
  ...
  // Column separators
  const sep1 = new Image({ url: WHITE_PX, x: VAL_X - 12, y: START_Y, width: 1, height: AVAIL_H, alpha: 0.10 })
  sep1.borderWidth = 0
  jsmaf.root.children.push(sep1)

  const sep2 = new Image({ url: WHITE_PX, x: HINT_X - 12, y: START_Y, width: 1, height: AVAIL_H, alpha: 0.07 })
  sep2.borderWidth = 0
  ...
  for (let s = 0; s < VISIBLE; s++) {
    ...
    const bg = new Image({ url: WHITE_PX, x: PAD_X, y: bY, width: ROW_W, height: BTN_H, alpha: 0.07 })
    bg.borderColor = 'rgba(120,200,255,0.16)'; bg.borderWidth = 1
    slotBgs.push(bg); jsmaf.root.children.push(bg)

    const bar = new Image({ url: CYAN_PX, x: PAD_X, y: bY, width: 4, height: BTN_H, alpha: 0.45 })
    bar.borderWidth = 0
    slotBars.push(bar); jsmaf.root.children.push(bar)

    const sec = new jsmaf.Text(); sec.text = ''; sec.x = PAD_X + 12; sec.y = bY + 8; sec.style = 'secbadge'
    slotSecs.push(sec); jsmaf.root.children.push(sec)

    const lbl = new jsmaf.Text(); lbl.text = ''; lbl.x = PAD_X + 12; lbl.y = bY + 28; lbl.style = 'muted'
    slotLabels.push(lbl); jsmaf.root.children.push(lbl)

    const arr = new jsmaf.Text(); arr.text = ''; arr.x = VAL_X - 22; arr.y = bY + 26; arr.style = 'arrow'
  ...
  arrowUp.x = SW / 2 - 68; arrowUp.y = HEADER_H + 2; arrowUp.style = 'scroll'; arrowUp.visible = false
  ...
  fh.x = SW / 2 - 260; fh.y = SH - FOOTER_H + 15; fh.style = 'footer'
  ...
  function renderRows () {
    for (let s = 0; s < VISIBLE; s++) {
      ...
      // Colour the left bar for toggles
      if (o.type === 'toggle' && sel) {
        // bar image itself doesn't change, but we use a neutral bar
      ...
      slotArrows[s]!.style = sel ? 'arrsel' : 'arrow'
